...
% Constantes globales (sin parámetros).
...
% Comandos con 1 o más parámetros para aplicar formato.

\newcommand{\sub}[2]{{{#1}_\textsl{{#2}}}} % #1 subíndice #2 (cuando #2 es una abreviación)
...
\newcommand{\inBrackets}[1]{{ \begin{bmatrix} \displaystyle #1 \end{bmatrix} }} % #1 entre corchetes
\newcommand{\inParentheses}[1]{\left(#1\right)} % #1 entre paréntesis
\newcommand{\inQuotes}[1]{``#1''} % #1 entre comillas
\newcommand{\cusTi}[1]{\noindent\textbf{#1}} % Deprecado
\newcommand{\cusTe}[1]{\vspace{2mm}\\\text{\hspace{\the\parindent}}#1} % Deprecado
\newcommand{\noTi}[1]{\text{\hspace{\the\parindent}}#1} % Deprecado
...
\DeclareMathOperator{\mse}{ecm} % Error cuadrático medio (Mean Squared Error)
\DeclareMathOperator{\lcm}{mcm} % Mínimo común múltiplo (Least Common Multiple)
\DeclareMathOperator{\gcm}{mcd} % Máximo común divisor (Greatest Common Divisor)

% -----------------------
% OPERACIONES MATEMÁTICAS
% -----------------------

% Comandos con 1 o más parámetros para desarollar expresiones matemáticas típicas.

\newcommand{\fx}[2][f]{#1 \hspace{-0.5mm} \left( #2 \right)} % #1 en función de #2
\newcommand{\deriv}[2][]{\frac{\dif #1}{\dif #2}} % Derivada de [#1] con respecto de #2
...
\newcommand{\convol}[2][h]{\displaystyle \int_{-\infty}^\infty \fx[#2]{\tau} \, \fx[#1]{t-\tau} \, \dif \tau} % Convolución de #2 con [#1]

% ---------
% ELEMENTOS
% ---------

% Comandos sin parámetros para denotar elementos matemáticos.
...
\newcommand{\setQ}{\mathbb{Q}} % Conjunto de los números racionales
...
\newcommand{\complexExp}{e^{\iu \omega t}} % Exponencial compleja en función del tiempo
\newcommand{\squaredL}{\mathcal{L}^2} % Espacio de Hilbert que contiene las funciones de energía finita

% --------
% NOTACIÓN
% --------

% Comandos con solo 1 parámetro para determinar los símbolos que se usan en ciertas notaciones.
...
% -----------------------
% CONSTANTES Y MAGNITUDES
% -----------------------

\newcommand{\weight}{\textsl{p}} % Peso
\newcommand{\cstcoulomb}{k_e} % Constante de Coulomb
\newcommand{\voltage}{\scale{1.2}{v}} % Tensión eléctrica
\newcommand{\current}{\scale{1.2}{i}} % Corriente eléctrica
\newcommand{\impedance}{\scale{1.2}{z}} % Impedancia eléctrica

% ---------------
% UNIDADES DEL SI
% ---------------

\newcommand{\kHz}[1][1]{\SI{#1}{\kilo\hertz}} % Frecuencia
\newcommand{\ohm}{\si{\ohm}} % Resistencia eléctrica

% --------------
% REDEFINICIONES
% --------------

\renewcommand{\arg}{\mathop{\mathrm{arg}}\mkern2mu} % Argumento, ya sea para denotar valor de entrada o ángulo
...
\NewDocumentCommand{\radpers}{o}{
    \IfNoValueTF{#1}{\si{\radian \per \second}}{\SI{#1}{\radian \per \second}}
}